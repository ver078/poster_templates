...
Tell us about the digital tools you use

This poster template is built with modifications from the `posterdown` package. You will need this 
installed if you wish to build your own poster. See the [GitHub repo](https://github.com/brentthorne/posterdown)
for more details, or use `remotes::install_github("brentthorne/posterdown")` to dive straight in. 
# Methods
...
```{r standard-plot, out.width='80%', fig.align='center', fig.cap='Great figure!'}
plot(mtcars[1:2])
```

```{r anim, fig.align='center', cache=TRUE, out.width="80%"}
